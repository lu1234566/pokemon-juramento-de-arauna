...
restore_overlays() {
    for file in "${overlay_files[@]}"; do cp "$overlay_backup_dir/$file" "$file"; done
...
make MODERN=1 BUILD_DIR="build/arauna-en" FILE_NAME="pokemon-juramento-de-arauna-en" CPP="$cpp" "$@"
