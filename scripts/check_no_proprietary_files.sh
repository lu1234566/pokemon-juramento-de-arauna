...
while IFS= read -r path; do
    case "$path" in
        data/mb_berry_fix.gba|data/mb_colosseum.gba|data/mb_ereader.gba)
            ...
            continue
            ;;
        graphics/arauna/arauna_sprites_gba_export.zip)
            # Tracked authoring asset: the Arauna sprite export the fakemon
            # graphics are curated from. Not read by the build.
            continue
            ;;
