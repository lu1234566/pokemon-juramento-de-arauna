...
Activate from the newest green `main` after the currently blocked GitHub Actions queue recovers. Wire this exact tool into the shared residue runner, generate `src/strings.c`, then require deterministic `--check` plus a successful full custom Emerald ROM build on the final head before merge.
