...
| Onde a Jornada Nasce | `MUS_ARAUNA_VILA_AMANHECER` | `mus_arauna_vila_amanhecer.mid` | `arauna_ost` | sim | `AraunaMapLab`, `AraunaPlayerHouse` | `MUS_FORTREE` | IMPLEMENTADA |
| Terra de Arauna | `MUS_ARAUNA_ROUTE` | `mus_arauna_route.mid` | `arauna_ost` | sim | `AraunaMistRoute` | `MUS_ROUTE119` | IMPLEMENTADA |
...
| Sempre Um Passo à Frente | `MUS_ARAUNA_CIRO` | `mus_arauna_ciro.mid` | `arauna_ost` | sim | 18 cenas de encontro de rival (11 mapas) | `MUS_ENCOUNTER_MAY` / `_BRENDAN` | IMPLEMENTADA |
| Rival à Altura | `MUS_ARAUNA_CIRO_BATTLE` | `mus_arauna_ciro_battle.mid` | `arauna_ost` | sim | Ciro (IDs Arauna + classe Rival) | `MUS_VS_RIVAL` | IMPLEMENTADA |
...
| Progresso a Qualquer Custo | `MUS_ARAUNA_HORIZON` | `mus_arauna_horizon.mid` | `arauna_ost` | sim | `AraunaResearchCenter`; encontro de agente Aqua/Horizonte | `MUS_OCEANIC_MUSEUM`, `MUS_ENCOUNTER_AQUA` | IMPLEMENTADA |
| Máquina de Controle | `MUS_ARAUNA_HORIZON_BATTLE` | `mus_arauna_horizon_battle.mid` | `arauna_ost` | sim | Agentes/admins/líder **Aqua** + `TRAINER_ARAUNA_TECH_AGENT` | `MUS_VS_AQUA_MAGMA` (**só o lado Aqua**) | IMPLEMENTADA |
...
| Algo Não Deveria Estar Aqui | `MUS_ARAUNA_STORY_BOSS` | `mus_arauna_story_boss.mid` | `arauna_ost` | sim | `TRAINER_ARAUNA_MARE_TRIAL`, `TRAINER_ARAUNA_UIVO_TRIAL` | — (novo) | IMPLEMENTADA |
| Vozes Antes dos Homens | `MUS_ARAUNA_LEGEND_ENCOUNTER` | `mus_arauna_legend_encounter.mid` | `arauna_ost` | sim | `AraunaFirstLinkChamber` (santuário) | `MUS_SEALED_CHAMBER` | IMPLEMENTADA |
...
| The Sapphire Crypt | `MUS_ARAUNA_SAPPHIRE_CRYPT` | `mus_arauna_sapphire_crypt.mid` | `arauna_ost` | sim | `AraunaFirstLinkRuin` | `MUS_SEALED_CHAMBER` | IMPLEMENTADA |

### Decisão documentada — The Sapphire Crypt

O briefing pediu escolha narrativa, não pelo título. A faixa é a mais lenta do
pacote (71 BPM) e a de textura mais escura e estática. Entre os locais
candidatos, `AraunaFirstLinkRuin` é a **ruína antiga jogável** do vertical
slice, e estava usando `MUS_SEALED_CHAMBER` vanilla — ou seja, era exatamente um
local importante *sem tema próprio*. A câmara adjacente
(`AraunaFirstLinkChamber`) ficou com *Vozes Antes dos Homens*, o que cria a
progressão ruína → santuário → aparição sem repetir música.
...
Os treinadores de história de Arauna reutilizam classes vanilla genéricas: Ciro
é `Pkmn Ranger`, o agente do Horizonte é `Scientist Frlg`, e os dois trials de
história são `Expert`. Rotear por classe arrastaria treinadores comuns junto.
Por isso `GetBattleBGM()` testa primeiro os **IDs** `TRAINER_ARAUNA_*` e só
depois cai no `switch` por classe. O bloco por ID é ignorado em Frontier e
Trainer Hill, onde os IDs têm outro significado.
...
- Toda a suíte `repository-safety` passa, incluindo
  ...
  `map.json`).
- **ROM completa não linkada neste ambiente** (sem toolchain ARM disponível), e
...
2. **Build da ROM e teste in-game** — falta compilar com toolchain ARM e validar
   os 21 contextos da seção 39 e os 2 loops completos da seção 40.
